...
public class ManageSeatTypeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            e.printStackTrace();
            request.setAttribute("error", "Error loading seat types: " + e.getMessage());
            request.getRequestDispatcher("/pages/manager/manage-seat-type.jsp").forward(request, response);
        } finally {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
                ...
                if (seatIds == null || seatIds.length == 0 || seatType == null) {
                    ...
                    if (!seatType.equals("NORMAL") && !seatType.equals("VIP") && !seatType.equals("COUPLE")) {
                        ...
                        if (seatIdList.isEmpty()) {
                            ...
                            success = seatsRepo.updateSeatTypesInBatch(seatIdList, seatType);
                            message = success ? "Updated " + seatIdList.size() + " seat(s) to " + seatType
            ...
            if (success) {
                response.sendRedirect(request.getContextPath() + "/branch-manager/manage-seat-type?roomId=" + roomId
                        + "&success=" + message);
            } else {
                response.sendRedirect(request.getContextPath() + "/branch-manager/manage-seat-type?roomId=" + roomId
                        + "&error=" + message);
            ...
            response.sendRedirect(
                    request.getContextPath() + "/branch-manager/manage-seat-type?error=Invalid ID format");
        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect(
                    request.getContextPath() + "/branch-manager/manage-seat-type?error=" + e.getMessage());
        } finally {
            if (branchDbContext != null) {
                branchDbContext.closeConnection();
            }
