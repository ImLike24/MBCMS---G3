...
<head>
    ...
    <title>LOSS Laptop - Đăng nhập</title>

    <link href="${pageContext.request.contextPath}/web/img/logo/favicon/DC-Symbol.png" rel="icon"/>

    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Roboto', sans-serif; background: linear-gradient(135deg, #9ACBD0 0%, #006A71 100%); min-height: 100vh; display: flex; align-items: center; justify-content: center; overflow: hidden; }
        .container { display: flex; width: 100%; max-width: 950px; height: 100vh; max-height: 700px; background: white; border-radius: 15px; box-shadow: 0 15px 30px rgba(0, 0, 0, 0.1); overflow: hidden; position: relative; }
        .left-panel { flex: 1; background: linear-gradient(135deg, #D84040 0%, #667eea 100%); position: relative; display: flex; align-items: center; justify-content: center; overflow: hidden; }
        ...
        .logo img { width: 60px; height: 60px; object-fit: contain; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.3)); transition: transform 0.3s ease; }
        ...
        .right-panel { flex: 1; padding: 60px 50px; display: flex; flex-direction: column; justify-content: center; background: white; position: relative; }
        .signin-header { margin-bottom: 40px; text-align: center; }
        .signin-title { font-size: 32px; font-weight: 700; color: #1a202c; margin-bottom: 8px; }
        .signin-subtitle { color: #718096; font-size: 16px; }
        .signin-form { width: 100%; max-width: 380px; margin: 0 auto; }
        .form-row { display: flex; }
        .form-group { margin-bottom: 24px; }
        .form-label { display: block; margin-bottom: 8px; color: #4a5568; font-weight: 500; font-size: 14px; }
        .form-input { width: 100%; padding: 14px 16px; border: 2px solid #e2e8f0; border-radius: 10px; font-size: 16px; transition: all 0.3s ease; outline: none; background: #fafafa; }
        .form-input:focus { border-color: #4c51bf; box-shadow: 0 0 0 3px rgba(76, 81, 191, 0.1); background: white; transform: translateY(-1px); }
        .forgot-password-group { display: flex; justify-content: flex-end; margin-bottom: 32px; }
        .forgot-password-link { color: #FFACAC; text-decoration: none; font-size: 14px; font-weight: 500; transition: color 0.3s ease; }
        .forgot-password-link:hover { color: #059669; text-decoration: underline; }
        .signin-btn { width: 100%; padding: 16px; background: linear-gradient(135deg, #FFACAC 0%, #FFBFA9 100%); color: white; border: none; border-radius: 10px; font-size: 16px; font-weight: 600; cursor: pointer; transition: all 0.3s ease; margin-bottom: 32px; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.2); }
        .signin-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(16, 185, 129, 0.3); }
        .signin-btn:active { transform: translateY(0); }
        .signup-link-group { text-align: center; margin-bottom: 32px; }
        .signup-link { color: #718096; text-decoration: none; font-size: 14px; }
        .signup-link strong { color: #FFACAC; font-weight: 600; }
        .signup-link:hover strong { color: #059669; }
        .show-password { display: flex; margin-right: 150px; }
        .error-message { background: #fed7d7; color: #c53030; padding: 12px 16px; border-radius: 8px; font-size: 14px; margin-bottom: 24px; text-align: center; border: 1px solid #feb2b2; animation: shake 0.5s ease-in-out; box-shadow: 0 2px 4px rgba(197, 48, 48, 0.1); }
        @keyframes shake { 0%, 100% { transform: translateX(0); } 25% { transform: translateX(-5px); } 75% { transform: translateX(5px); } }
        .footer-links { display: flex; justify-content: center; gap: 24px; margin-top: 20px; }
        .footer-links a { color: #a0aec0; text-decoration: none; font-size: 12px; transition: color 0.3s ease; }
        .footer-links a:hover { color: #4c51bf; }
        .decorative-elements { position: absolute; top: 0; right: 0; width: 200px; height: 200px; background: linear-gradient(135deg, #10b981, #06d6a0); opacity: 0.08; clip-path: polygon(100% 0, 0 0, 100% 100%); }
        ...
        @media (max-width: 768px) { .container { flex-direction: column; height: auto; min-height: 100vh; max-height: none; } .left-panel { height: 250px; flex: none; } .right-panel { padding: 40px 24px; } .signin-form { max-width: 100%; } .signin-title { font-size: 28px; } }
        @media (max-width: 480px) { .right-panel { padding: 30px 20px; } .signin-title { font-size: 24px; } }
    </style>
</head>
<body>
<div class="container">
    ...
    <div class="right-panel">
        ...
        <div class="signin-header">
            ...
            <p class="signin-subtitle">Chào mừng bạn trở lại!</p>
        </div>
