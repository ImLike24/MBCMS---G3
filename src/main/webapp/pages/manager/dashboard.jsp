...
<head>
    ...
    <title>Dashboard Quản lý chi nhánh - MBCMS</title>
    ...
    <style>
        ...
        .status-label { display: flex; align-items: center; gap: 12px; font-size: 1rem; }
        .status-badge {
            width: 12px;
            height: 12px;
            border-radius: 50%;
        ...
        .badge-success { background-color: var(--success-color); color: var(--success-color); }
        .badge-warning { background-color: #ffc107; color: #ffc107; }
        .status-count { font-weight: 700; font-size: 1.3rem; }
        .filter-bar {
            background-color: var(--card-bg);
            padding: 16px 20px;
            border-radius: 12px;
        ...
        .filter-bar label { color: var(--text-muted); font-size: 0.85rem; margin-bottom: 6px; display: block; }
...
<main>
    <div class="container-fluid">
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
            <div>
                <h3 class="fw-bold text-white mb-1">Dashboard chi nhánh</h3>
                <p class="text-muted mb-0">
                    Tổng quan tháng
                    <fmt:formatDate value="<%= new java.util.Date() %>" pattern="MM/yyyy"/>
                    <c:choose>
                        <c:when test="${selectedBranchId != null}">
                            — <span class="text-white">một chi nhánh đã chọn</span>
                        </c:when>
                        <c:otherwise>
                            — <span class="text-white">tất cả chi nhánh bạn quản lý</span>
                        </c:otherwise>
                    </c:choose>
                </p>
            </div>
        ...
        <form class="filter-bar" method="get" action="${pageContext.request.contextPath}/branch-manager/dashboard">
            <div>
                <label for="branchId">Chi nhánh</label>
                <select name="branchId" id="branchId" class="form-select">
                    <option value="all" ${selectedBranchId == null ? 'selected' : ''}>Tất cả chi nhánh</option>
        ...
        <div class="kpi-grid">
            <div class="kpi-card">
                ...
                <div class="kpi-trend ${dashboard.kpi.revenueIncreased ? 'trend-up' : 'trend-down'}">
                    ...
                    <span>
                        <fmt:formatNumber value="${dashboard.kpi.revenueChange}" maxFractionDigits="1"/>% so với tháng trước
                    </span>
                </div>
            </div>
            <div class="kpi-card">
                ...
                <div class="kpi-trend ${dashboard.kpi.ticketsChange >= 0 ? 'trend-up' : 'trend-down'}">
                    ...
                    <span>${dashboard.kpi.ticketsChange >= 0 ? '+' : ''}${dashboard.kpi.ticketsChange} vé so với tháng trước</span>
                </div>
            </div>
            <div class="kpi-card">
                ...
                <div class="kpi-trend">
                    <span class="text-muted">Toàn hệ thống (tháng này)</span>
                </div>
            </div>
            <div class="kpi-card">
                <div class="kpi-title"><i class="fas fa-store"></i> Chi nhánh hoạt động</div>
                <div class="kpi-value">
                    <span class="${dashboard.kpi.activeBranches < dashboard.kpi.totalBranches ? 'text-danger' : 'text-success'}">
                        ${dashboard.kpi.activeBranches}/${dashboard.kpi.totalBranches}
                    </span>
                </div>
                <div class="kpi-trend">
                    <span class="text-muted">Trong phạm vi chi nhánh bạn quản lý</span>
                </div>
            </div>
        </div>
        ...
        <div class="charts-grid">
            <div class="chart-card">
                <h3>Doanh thu theo chi nhánh (tháng này)</h3>
                <div class="chart-container">
                    <canvas id="revenueBranchChart"></canvas>
                </div>
            </div>
            <div class="chart-card">
                <h3>Top phim (tháng này)</h3>
                <div class="chart-container">
                    ...
                    <table>
                        ...
                        <c:forEach var="item" items="${dashboard.recentTransactions}">
                            <tr>
                                ...
                                <td class="text-truncate" style="max-width: 150px;">${item.movieTitle}</td>
                        ...
                        <c:if test="${empty dashboard.recentTransactions}">
                            <tr>
                                <td colspan="6" style="text-align: center; color: var(--text-muted); padding: 30px;">
                                    Chưa có giao dịch trong phạm vi đã lọc.
                                </td>
                            </tr>
                        </c:if>
                        </tbody>
            ...
            <div class="list-card">
                ...
                <div style="margin-top: 25px; text-align: center;">
                    <a href="${pageContext.request.contextPath}/branch-manager/manage-showtimes"
                       style="color: var(--primary-color); text-decoration: none; font-weight: bold; border: 1px solid var(--primary-color); padding: 10px 20px; border-radius: 5px; display: inline-block;">
                        Quản lý suất chiếu <i class="fas fa-arrow-right ms-2"></i>
                    </a>
                </div>
            </div>
        </div>
...
<script>
    ...
    const revenueByBranchData = ${revenueByBranchJson};
    ...
    new Chart(ctxMovies, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['#d96c2c', '#ffce56', '#4bc0c0', '#36a2eb', '#9966ff'],
                borderWidth: 0
            }]
        },
        options: {
